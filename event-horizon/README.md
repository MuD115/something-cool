...
**Easiest:** open `event-horizon.html` (the single-file build) by double-clicking it. It needs
an internet connection, because three.js loads from jsDelivr, but no server or install.

**From source:** serve this folder with any static server, then open the printed URL:
...
Opening `index.html` straight from disk won't work, because browsers block its separate
module files on `file://`. After changing anything in `src/`, rebuild the single file with
`python3 event-horizon/build.py`.
...
- `event-horizon.html`: single-file build of everything below, generated by `build.py`
